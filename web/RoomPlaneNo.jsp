    ...
    <body><%
        if ((request.getParameter("submit") == null)) {
            ...
            try {
                ...
                rs = statement.executeQuery(" select rs_roomno,rs_statusno,rs_available from roomstatus order by rs_roomno");
                while (rs.next()) {
                    // h = rs.getString(1);
        ...
        <%
            ...
            if ((request.getParameter("submit") != null)) {
                ...
                try {
                    ...
                    rs = statement.executeQuery(" select rs_roomno,rs_statusno,rs_available from roomstatus where rs_roomno ='" + code + "'");
                    while (rs.next()) {
                        // h = rs.getString(1);
                        roomno[k] = rs.getString(1);
                        rstatus[k] = rs.getString(2);
        ...
        %>
        <form method="post" action="RoomPlane1.jsp">
            <table>
                <tr><td>Room No</td><td><input type="text" name ="code" value="<%=code%>"/></td> <td><input type="text" name="s"></td>
</tr>

                <tr><td colspan="2" align="center"><input type="submit" name="submit" value="Search"></input>
