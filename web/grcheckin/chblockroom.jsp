...
<html>
    <head>
        ...
        <SCRIPT TYPE="text/javascript">
            function submitAndClose(acc,dtl) {
                window.opener.location.href="namelist.jsp?bkacc="+acc+"&dtl="+dtl;
    ...
    <body>
        <div class="content">
            <div class="page-header offset1">
                <h1>Select new room type and bed room </h1>
            </div>
            <div class="row">
                ...
                <div class="span16 columns">
                    ...
                    %>
                    <!-- content here -->
                    <form class="form-stacked offset1" name="frmMain" method="post"  onsubmit="chooseRoom()">
                        <div class="row"  >
                                ...
                                <div class="clearfix" width="350" style="width: 350px" >
                                    ...
                                    <div class="input"  >                                   
                                        <select class="span3" id="room" name="room" onclick="getRoom1(this.value)" width="250" style="width: 250px"> 
                                            <%
                                                String sql = "select ri_room,ri_location,ri_type,ri_bedtype from roominfo where ri_room not in "
                                                        + "( select gf_roomno from gfolio fol inner join gaccount ac on ga_accno = gf_accno "
                                                        + " where ga_accstat ='I'  and '" + (String) request.getParameter("arr") + "' < ga_departure and gf_roomno is not null) "
                                                        + "    and ri_room not in  (select bl_roomno from block  where  bl_arrdate between '" + (String) request.getParameter("arr") + "' and '" + dep + "' "
                                                        + " or '" + (String) request.getParameter("arr") + "' between bl_arrdate and bl_depdate-1 ) and ri_room not in ( "
                                                        + "  select ro_id from roomooo   where  '" + (String) request.getParameter("arr") + "' between ro_start and ro_end "
                                                        + "    or ro_start between '" + (String) request.getParameter("arr") + "' and '" + dep + "' )   order by ri_room asc ";
                                                ...
                                                while (rs.next()) {
                                            %>
                                            <option value="<%=rs.getString(1)%>+<%=rs.getString(3)%>+<%=rs.getString(4)%>"><%=ll1q%> <%=rs.getString(1)%><%=llq%><%=rs.getString(2)%><%=rtt%><%=rs.getString(3)%><%=btt%><%=rs.getString(4)%> </option>
                                            <%
                                                }
                                            %>
                                ...
                                function chooseRoom(){
                                  ...
                                  var p=window.opener;
                                             var dep='<%=(String) request.getParameter("dep")%>';
                                                 var seq='<%=(String) request.getParameter("seq")%>';
                                                   var arr='<%=(String) request.getParameter("arr")%>';
                                                     var acc='<%=(String) request.getParameter("acc")%>';
                                    var dtl='<%=(String) request.getParameter("dtl")%>';
                                    
                                    if(p.$('#roomNo')!=null){
                                                    
                                                   
                                        var myString =$('#room').val();
                                        var mySplitResult = myString.split("+");
                                       
                                         var room1 = mySplitResult[0];
                                        document.frmMain.bt.value = mySplitResult[1];
                                        var rt=mySplitResult[1];
                                        document.frmMain.rt.value = mySplitResult[2];
                                        var bt=mySplitResult[2];
                                              location.href = 'chblockroom_1.jsp?acc='+acc+'&bt='+bt+'&rt='+rt+'&arr='+arr+'&dep='+dep+'&room='+room1+'&dtl='+dtl+'&seq='+seq+'';

                                      //  alert(document.frmMain.bt.value);
                            ...
                            <script>
                                function getRoom1(str){
                                    ...
                                    if (lo=="none" ){
                                        ...
                                        location.href = 'chblock.jsp?acc='+acc+'&bt='+bt+'&rt='+rt+'&arr='+arr+'&dep='+dep+'&dtl='+dtl+'&seq='+seq+'';
                            ...
                            <script>
                                function getRoom(){
                                    ...
                                    var st=$('#status').val();
                                    var ex=$('#exposure').val();
                                    $.ajax( '<%=request.getContextPath()%>/getroom.jsp?bt='+bt+'&rt='+rt+'&lo='+lo+'&st='+st+'&ex='+ex+'&arr='+arr+'&dep='+dep).done(function(data) { 
                            ...
                            <div class="span1">
                                <div class="clearfix">
                                    ...
                                    <div class="input">
                                        ...
                                        <input type="button" class="btn info" onclick="chooseRoom()"   value="+"/>
                                    </div>
                                </div>
                            </div>
                            ...
                            <table id="blroom">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><a class="btn info" href="blockroom_2.jsp?acc=<%=request.getParameter("acc")%>&dtl=<%=request.getParameter("dtl")%>&seq=<%=rs.getString(1)%>&bt=<%=request.getParameter("bt")%>&rt=<%=request.getParameter("rt")%>&arr=<%=request.getParameter("arr")%>&dep=<%=request.getParameter("dep")%>" >-</a></td>
                                    </tr>
                                    <%
                                        }
                                    %>
                    ...
                    <div class="row">
                        <div class="actions">
                            <button class="btn primary offset-two-thirds" onclick="submitAndClose(<%=acc%>,<%=dtl%>)">Close</button>
                        </div>
                    </div>
                </div>
            </div>
