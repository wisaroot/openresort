...
<%!
    private void validate(BlockRoomGr blockRoom, List errorList) {
     ...
     /*   if (Converter.isEmpty(blockRoom.getDeparture())) {
        ...
        if (Converter.isEmpty(blockRoom.getrt())) {
            //  errorList.add("rt is empty");
        }
    }
    ...
    private boolean addBlockRoom(String user1,String pw,String url,String driver,BlockRoomGr blockRoom, WebUser user, List errorList) throws SQLException {
       ...
       String updateNameListSQL = "update namelist set nl_roomno=? where nl_accno=? and nl_dtlseq=? and nl_seq=? ";
        ...
        try {
           ...
           preparedStatementInsert = dbConnection.prepareStatement(updateNameListSQL);
            ...
            preparedStatementInsert.setInt(2, Converter.parseInt(blockRoom.getAccountId()));
            preparedStatementInsert.setInt(3, Converter.parseInt(blockRoom.getDtl()));
            preparedStatementInsert.setInt(4, Converter.parseInt(blockRoom.getseq()));
...
<%
    ...
    if (errorList.size() > 0) {
        response.sendRedirect("blockroom.jsp?acc=" + blockRoom.getAccountId() + "&dtl=" + blockRoom.getDtl() + "&arr=" + blockRoom.getArrival() + "&dep=" + blockRoom.getDeparture() + "&bt=" + request.getParameter("bt") + "&rt=" + request.getParameter("rt") + "&seq=" + request.getParameter("seq"));
    } else {
