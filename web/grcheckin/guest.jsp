...
<html>
    <head>
        ...
        <script type="text/javascript">
            function closeWindow(acc,bkacc,dtl,seq){
                
                    var p=window.opener;                   
                    if(p.$('#details')!==null){
                        p.$('#details').load("newnamelist.jsp?acc="+acc+"&bkacc="+bkacc+"&dtl="+dtl+"&seq="+seq);
                        window.close();
                    }
        ...
        <div class="content">
            ...
            <div class="row">
                ...
                <div class="span15 columns offset1">
                    ...
                    <a class="btn primary" href="<%=request.getContextPath()%>/guest/newguest.jsp?bkacc=<%=request.getParameter("bkacc")%>&dtl=<%=request.getParameter("dtl")%>">New Guest</a>
                    <br/>
                    <br/>
                    <table class="zebra-striped" id="tbRateCode">
                        ...
                        <tbody>
                            <%
                                ...
                                String selectSQL = "SELECT gaccount.ga_accno, title.t_descr, guestname.gn_fname, guestname.gn_mname, guestname.gn_lname FROM public.gaccount right join public.guestname on (ga_guestno=gn_guestno) left join public.title on (gn_titleno=t_id) " + (query != null && !query.equals("") ? " where (gn_fname || gn_mname || gn_lname) ilike ('%" + query.replaceAll("\\s+","") + "%')" : "") + " order by ga_accno asc limit " + pgLimit + " offset " + ((iPg - 1) * pgLimit);
                                
                                String maxRateSQL = "select count(*) from gaccount right join guestname on (ga_guestno=gn_guestno)" + (query != null  && !query.equals("")? " where  (gn_fname || gn_mname || gn_lname) ilike ('%" + query.replaceAll("\\s+","") + "%')" : "");
                                try {
                            ...
                            <tr>
                                ...
                                <td><a class="btn info" href="<%=request.getContextPath()%>/guest/editguest.jsp?acc=<%=rs.getString(1)%>">Edit</a></td>
                                <td><a class="btn info" href="#" onclick="closeWindow(<%=rs.getString(1)%>,<%=nameList.getBookingAccountId()%>,<%=nameList.getDtl()%>,<%=nameList.getSeq()%>)">Select</a></td>
                            </tr>
                            <%
                                    }
                        ...
                        <tfoot>
                            <tr><td colspan="2">
                                    <div class="pagination">
                                        ...
                                        <ul>
                                            <li class="prev <% if (pg == null || pg.equals("1")) {%> disabled<%}%>"><a href="<%=request.getContextPath() + "/guest/guest.jsp?pg=" + (pg != null ? Integer.parseInt(pg) - 1 : "1") + (query != null ? "&q=" + query : "")%>">&larr; Previous</a></li>
                                            <%
                                            ...
                                            %>
                                            <li class="<% if (i == iPg) {%>active<% }%>"><a href="<%=request.getContextPath() + "/guest/guest.jsp?pg=" + i + (query != null ? "&q=" + query : "")%>"><%=i%></a></li>
                                            <%
                                                }
                                            %>
                                            <li class="next <% if (iPg >= maxPg) {%> disabled <% }%>"><a href="<%=request.getContextPath() + "/guest/guest.jsp?pg=" + (pg != null ? Integer.parseInt(pg) + 1 : "2") + (query != null ? "&q=" + query : "")%>">Next &rarr;</a></li>
                                        </ul>
                                    </div></td>
                                <td colspan="2">
                                    <div style="height:36px;margin:18px 0;"><ul style="float:left;margin:0;"><li style="display:inline;">
                                                <form action="<%=request.getContextPath() + "/guest/guest.jsp"%>">
