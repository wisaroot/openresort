...
<%!
    ...
    private boolean saveBooking(String user1,String pw,String url,String driver,Guest guest,WebUser user, List errorList) throws SQLException {
        ...
        String insertGAccountSQL = "INSERT INTO gaccount(ga_accstat, ga_gsttype, ga_rsvtypeno, ga_bookby, ga_acctype, ga_walkin, ga_guestno,ga_agentno, ga_orino, ga_arrival, ga_arrtime, ga_arrfrom, ga_arrby, ga_departure, ga_deptime, ga_depto, ga_depby, ga_fpatno, ga_recdate, ga_userno, ga_coa,ga_voucher)VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?);";
        ...
        try {
            ...
            //ga_walkin,
            preparedStatementInsert.setString(6,"Y");
            //ga_guestno,
            ...
            preparedStatementInsert.setString(21,Converter.parseToYesNo(guest.getCoa()));
               preparedStatementInsert.setString(22,guest.getVoucher());
            preparedStatementInsert.executeUpdate();
            generatedKeys = preparedStatementInsert.getGeneratedKeys();
            int gAccountId;
            if (generatedKeys.next()) {
...
<%
    ...
    if (errorList.size() > 0) {
        ...
        response.sendRedirect("newguest.jsp");
    } else {
        
        if (saveBooking(user1, pw, url, driver,guest,user, errorList)) {
            
            response.sendRedirect("newcheckin.jsp?acc="+guest.getAccountId()+"&arr="+guest.getArrival()+"&dep="+guest.getDeparture());
        } else {
