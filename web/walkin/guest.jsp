                    ...
                    <table class="zebra-striped" id="tbRateCode">
                        ...
                        <tbody>
                            <%
                                ...
                                String selectSQL = "SELECT gaccount.ga_accno, title.t_descr, guestname.gn_fname, guestname.gn_mname, guestname.gn_lname,ga_arrival,ga_departure FROM public.gaccount left join public.guestname on (ga_guestno=gn_guestno) left join public.title on (gn_titleno=t_id) where " + (query != null && !query.equals("") ? "  (gn_fname || gn_mname || gn_lname) like ('%" + query.replaceAll("\\s+","") + "%') and " : "") + " gaccount.ga_walkin='Y' order by ga_accno asc limit " + pgLimit + " offset " + ((iPg - 1) * pgLimit);
                                
                                String maxRateSQL = "select count(*) from gaccount left join guestname on (ga_guestno=gn_guestno) where " + (query != null  && !query.equals("")? "   (gn_fname || gn_mname || gn_lname) like ('%" + query.replaceAll("\\s+","") + "%') and " : "")+" gaccount.ga_walkin='Y'";
                                try {
                     ...
                     <%--          <td><a class="btn info" href="newcheckin.jsp?acc=<%=rs.getString(1)%>&arr=<%=rs.getString(6)%>&dep=<%=rs.getString(7)%>">Walk In</a></td>
                      --%>      </tr>
                            <%
                                    }
