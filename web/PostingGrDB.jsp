...
//java.sql.Date date = new java.sql.Date(new java.util.Date().getTime());
    ...
    String[] groupno = new String[TOTALINH];
    ...
    rs = statement.executeQuery(" select gf_folno,gf_accno,gf_roomno,ga_arrival,ga_departure,ga_grpno from gfolio left join gaccount on (gfolio.gf_accno=gaccount.ga_accno)  where ga_accstat = 'I' and ga_acctype = 'G' and ga_guestno is  null  ");
    while (rs.next()) {
        ...
        groupno[i] = rs.getString(6);
        i++;
    }
    int j = 0, k = 0;
    while (j < i) {
        rs = statement.executeQuery(" select gri_grpname  from gaccount left join groupinfo on (groupinfo.gri_grpno=gaccount.ga_grpno)  where ga_grpno =  '" + groupno[j] + "'");
        while (rs.next()) {
    ...
    while (y < TOTALINH) {
        ...
        cell.add(0, folno[y]);
        cell.add(1, name[y]);
        cell.add(2, arrival[y]);
        cell.add(3, departure[y]);
        cell.add(4, fol1[y]);
        cell.add(5, fol2[y]);
