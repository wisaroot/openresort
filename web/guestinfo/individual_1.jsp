...
<table  class="zebra-striped" >
    ...
    <tbody>
          ...
          //  errorList = new ArrayList<String>();
           // session.setAttribute("errors" + session.getId(), errorList);
            ...
            PreparedStatement preparedStatementSelect = null;
                  ...
                  String selectSQL = "   select ga_accno, case when bcf_cfmno >0 then bcf_cfmno else 0 end , case when gf_roomno is not null then gf_roomno else '' end,ga_accstat,case when (gn_fname||' '||gn_lname) is not null then gn_fname||' '||gn_lname else '' end , case when (ga_grpno is not null and coalesce(ga_guestno,0) = 0) "
               +"      then   gri_grpname   else ''  end ,ga_arrival,  ga_arrtime,ga_departure, "
             +"    ga_deptime,case when a_name is not null then a_name else '' end ,case when n_descr is not null then n_descr else '' end,case when gi_company is not null then gi_company else '' end, "
            ...
            +"        left join   guestinfo on gi_guestno = ga_guestno left join nationality on  nationality.n_id=gi_natno  "
             +"       GROUP BY gaccount.ga_accno,bconfirm.bcf_cfmno,gfolio.gf_roomno,gaccount.ga_accstat, "
       ...
       /*     String selectSQL = "  select ga_accno, case when bcf_cfmno >0 then bcf_cfmno else 0 end , case when gf_roomno is not null then gf_roomno else '' end, ga_accstat,gn_fname||' '||gn_lname as guestname, case when (ga_grpno is not null and coalesce(ga_guestno,0) = 0) "
                    + "    then   gri_grpname   else ''  end ,ga_arrival,  ga_arrtime,ga_departure, "
                    + "   ga_deptime,case when a_name is not null then a_name else '' end ,n_descr,case when gi_company is not null then gi_company else '' end,case when b_qty > 0 then b_qty else '0' end,case when b_inhqty > 0 then b_inhqty else '0' end,case when b_seq >= 0 then b_seq+1 else '0' end   , "
                    + " gf_folno,b_accno,ga_acctype,ga_walkin from gaccount left join   gfolio  on ga_accno=gf_accno  left join booking on gf_bkaccno=b_accno left join  BCONFIRM on b_accno=bcf_accno   left join guestname "
                    + "on guestname.gn_guestno=gaccount.ga_guestno "
                    + " left join agent on a_id = ga_agentno left join groupinfo on gri_grpno = ga_grpno"
                    + " left join   guestinfo on gi_guestno = ga_guestno left join nationality on  nationality.n_id=gi_natno  "
                    + "    GROUP BY gaccount.ga_accno,bconfirm.bcf_cfmno,gfolio.gf_roomno,gaccount.ga_accstat,"
...
//String selectSQL = "select gf_accno,bcf_cfmno,gf_roomno,ga_accstat,gn_fname||gn_lname,gri_grpname,ga_arrival,ga_departure,a_name,n_descr,gi_company,b_qty,b_inhqty,b_seq from search_guestinfo_i " + extractData(individual) + " order by gf_accno asc limit " + pgLimit + " offset " + ((iPg - 1) * pgLimit);
            ...
            String maxRateSQL = "  select count(*) from ( select ga_accno,bcf_cfmno,gf_roomno,ga_accstat,gn_fname||' '||gn_lname as guestname, "
           +"   case when (ga_grpno is not null and coalesce(ga_guestno,0) = 0)         then   gri_grpname   else null  "
                    +"   end,ga_arrival,ga_arrtime,ga_departure,ga_deptime,a_name,n_descr,gi_company,b_qty,b_inhqty,b_seq+1 as b_seq , "
+" gf_folno,b_accno,ga_acctype,ga_walkin from gaccount left join booking on ga_accno=b_accno left join  BCONFIRM on b_accno=bcf_accno "
                    + "left join gfolio on gf_accno=ga_accno left join guestname on guestname.gn_guestno=gaccount.ga_guestno "
+"  left join agent on a_id = ga_agentno left join groupinfo on gri_grpno = ga_grpno left join   guestinfo on gi_guestno = ga_guestno left join nationality "
                    + "on  nationality.n_id=gi_natno  "
                    + "GROUP BY gaccount.ga_accno,bconfirm.bcf_cfmno,gfolio.gf_roomno,gaccount.ga_accstat,guestname.gn_fname,guestname.gn_lname "
 +" ,gaccount.ga_grpno,gaccount.ga_guestno,groupinfo.gri_grpname,gaccount.ga_arrival,gaccount.ga_arrtime,gaccount.ga_departure,gaccount.ga_deptime,"
                    + "agent.a_name,nationality.n_descr,guestinfo.gi_company,booking.b_qty ,booking.b_inhqty ,gfolio.gf_folno,booking.b_accno,"
                    + "gaccount.ga_acctype,gaccount.ga_walkin,booking.b_seq  order by ga_accno asc ) as aa ";
// where (b_seq = (select max(b_seq) as seq from booking where b_accno =ga_accno)) or       b_accno IS NULL  
            try {
        ...
        <tr> 
         ...
         <td><%=rs.getString(2)%></td>
            ...
            <td>
                ...
                %>
                <button href="#" class="btn info" onclick="window.open('individual_2.jsp?acc=<%=rs.getString(1)%>&dtl=<%=rs.getString(1)%>', '_blank', 'height=700, width=1000');return false;"><%=rs.getString(1)%></button>
                <%
               ...
               <%
                ...
                %>  <a href="<%=request.getContextPath() + "/checkin/checkin.jsp?acc=" + rs.getString(1) + "&roomno=" +  rs.getString(3).trim() + "&gf=" + rs.getString(14)%>" class="btn info">....</a>
               <%
                    } else{ 
...
<%!
    public String extractData(Individual individual) {
        ...
        if (!Converter.isEmpty(individual.getFirst())) {
            data += extractStringField(data, "gn_fname", individual.getFirst());

        }
        if (!Converter.isEmpty(individual.getLast())) {
            data += extractStringField(data, "gn_lname", individual.getLast());
        }
     /*   if (!Converter.isEmpty(individual.getMiddle())) {
            data += extractStringField(data, "gn_mname", individual.getMiddle());
        }*/

        if (!Converter.isEmpty(individual.getNationality())) {
            data += extractStringField(data, "n_id", individual.getNationality());
        }
        if (!Converter.isEmpty(individual.getArrival())) {
            data += extractDateField(data, "ga_arrival", individual.getArrival());
        }
        if (!Converter.isEmpty(individual.getDeparture())) {
            data += extractDateField(data, "ga_departure", individual.getDeparture());
        }
    /*    if (!Converter.isEmpty(individual.getCompany())) {
            data += extractStringField(data, "gi_company", individual.getCompany());
        }*/
        if (!Converter.isEmpty(individual.getAgent())) {
            data += extractIntegerField(data, "a_id", individual.getAgent());
        }
        if (!Converter.isEmpty(individual.getGroup())) {
            data += extractStringField(data, "gri_grpname", individual.getGroup());
        }
        if (!Converter.isEmpty(individual.getRoomNo())) {
            data += extractStringField(data, "gf_roomno", individual.getRoomNo());
        }
        if (!Converter.isEmpty(individual.getAccountId())) {
            data += extractIntegerField(data, "gf_accno", individual.getAccountId());
        }
        if (!Converter.isEmpty(individual.getStatus())) {
            data += extractStringField(data, "ga_accstat", individual.getStatus());
        }
        return data;
    }
