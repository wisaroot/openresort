...
<%!
    private void validate(RequirementNote requirementNote, List errorList) {
        ...
        if (Converter.isEmpty(requirementNote.getFromDate())) {
            errorList.add("fromdate is empty");
        }
    }
    ...
    private boolean addRequirementNote(String user1, String pw, String url, String driver, RequirementNote requirementNote, List errorList) throws SQLException {
        ...
        String insertRequirementSQL = "UPDATE brequire SET  brq_eventtime=?, brq_enddate=?, brq_chargeflag=?, brq_deptno=?, brq_amount=?, brq_chargetype=?, brq_remark=? WHERE brq_accno=? and brq_reqno=? and brq_fromdate=?;INSERT INTO brequire(brq_accno, brq_reqno, brq_fromdate, brq_eventtime, brq_enddate, brq_chargeflag, brq_deptno, brq_amount, brq_chargetype, brq_remark) select ?, ?, ?, ?, ?, ?, ?, ?, ?, ? where not exists (select 1 from brequire where brq_accno=? and brq_reqno=? and brq_fromdate=?);";
        String insertNoteSQL = "UPDATE bnote SET bn_accno=?, bn_note=? WHERE bn_accno=?;INSERT INTO bnote(bn_accno, bn_note) select ?, ? where not exists (select 1 from bnote where bn_accno=?);";
...
<%
    ...
    if (errorList.size() > 0) {
        ...
        if (addRequirementNote(user1, pw, url, driver, requirementNote, errorList)) {
            // response.sendRedirect("../booking/bookingdetails.jsp?acc=" + requirementNote.getAccountId());
            response.sendRedirect("newrequirementnote.jsp?acc=" + requirementNote.getAccountId());
        } else {
            response.sendRedirect("newrequirementnote.jsp?acc=" + requirementNote.getAccountId());
        }
