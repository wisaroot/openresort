...
<%  String bk=request.getParameter("bkacc");
...
Guest guest,WebUser user, List errorList) throws SQLException {
...
<%
    ...
    if (errorList.size() > 0) {
       ...
       response.sendRedirect("newguest.jsp?acc="+ Integer.toString(gAccountId)+"&dtl=0&seq="+request.getParameter("seq")+"&bkacc="+bk);
     
    } else {
        if (saveBooking(user1, pw, url, driver,guest,user, errorList)) {
            CheckIn checkIn=(CheckIn)session.getAttribute("checkIn"+session.getId());
            response.sendRedirect("newcheckin.jsp?acc="+ Integer.toString(gAccountId)+"&dtl=0&seq="+seq+"&bkacc="+bk+"&roomno="+checkIn.getRoomNo());
        } else {
