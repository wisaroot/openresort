...
<%!
    private void validate(GFolio gFolio,List errorList){
        ...
        if(Converter.isEmpty(gFolio.getgFolioId()))
            errorList.add("gFolioId is empty");
    }
    private boolean updateGFolio(String user1,String pw,String url,String driver,GFolio gFolio,WebUser user,List errorList)throws SQLException{
        Connection dbConnection = null;
        PreparedStatement preparedStatementInsert = null;
        String updateGFolioSQL = "UPDATE gfolio SET gf_accno=?, gf_bkaccno=?, gf_bookseq=?, gf_roomno=?, gf_roomtypeno=?, gf_adult=?, gf_child=?, gf_comp=?, gf_compby=?, gf_rateno=?, gf_room=?, gf_service=?, gf_tax=?, gf_extrabed=?, gf_extbedserv=?, gf_extbedtax=?, gf_abf=?, gf_lunch=?, gf_dinner=?, gf_discount=?, gf_total=?, gf_discby=? WHERE gf_folno=?;";
       
        boolean isSave = false;
        try {
          dbConnection = JDBCTransaction.getDBConnection( user1, pw, url, driver, errorList);
            dbConnection.setAutoCommit(false);
            preparedStatementInsert = dbConnection.prepareStatement(updateGFolioSQL);
            ...
            preparedStatementInsert.setString(4,gFolio.getRoomNo());
            preparedStatementInsert.setString(5,gFolio.getRoomType());
            preparedStatementInsert.setInt(6,Converter.parseInt(gFolio.getAdult()));
            preparedStatementInsert.setInt(7,Converter.parseInt(gFolio.getChild()));
            preparedStatementInsert.setString(8,(gFolio.getCompliment()==null?"n":gFolio.getCompliment()));
            preparedStatementInsert.setString(9,gFolio.getCompBy());
            preparedStatementInsert.setString(10,gFolio.getRateCode());
            preparedStatementInsert.setDouble(11, Converter.parseDouble(gFolio.getRoom()));
            preparedStatementInsert.setDouble(12, Converter.parseDouble(gFolio.getService()));
            preparedStatementInsert.setDouble(13, Converter.parseDouble(gFolio.getTax()));
            preparedStatementInsert.setDouble(14, Converter.parseDouble(gFolio.getExtraBed()));
            preparedStatementInsert.setDouble(15, Converter.parseDouble(gFolio.getExtraBedService()));
            preparedStatementInsert.setDouble(16, Converter.parseDouble(gFolio.getExtraBedTax()));
            preparedStatementInsert.setDouble(17, Converter.parseDouble(gFolio.getAbf()));
            preparedStatementInsert.setDouble(18, Converter.parseDouble(gFolio.getLunch()));
            preparedStatementInsert.setDouble(19, Converter.parseDouble(gFolio.getDinner()));
            preparedStatementInsert.setDouble(20, Converter.parseDouble(gFolio.getDiscount()));
            preparedStatementInsert.setDouble(21, Converter.parseDouble(gFolio.getTotal()));
            preparedStatementInsert.setString(22, gFolio.getDiscountBy());
            preparedStatementInsert.setInt(23,Converter.parseInt(gFolio.getgFolioId()));
...
<%
    ...
    gFolio.setgFolioId((String)request.getParameter("gFolioId"));
    validate(gFolio, errorList);
   
    
    if(errorList.size()>0){
        response.sendRedirect("editcheckin.jsp?gf="+gFolio.getgFolioId());
    }else{
        if(updateGFolio(user1, pw, url, driver,gFolio,user, errorList)){
