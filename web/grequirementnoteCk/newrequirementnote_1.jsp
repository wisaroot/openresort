...
String cg=null;
  ...
  Date fromdate=Converter.parseDate(requirementNote.getFromDate()),enddate=Converter.parseDate(requirementNote.getEndDate());
  ...
  while ( fromdate.before(enddate) ||fromdate.equals(enddate) ) {
               ...
               insertGchargeSQL = "INSERT INTO gcharge(gc_accno, gc_reqno, gc_chgdate, gc_postflag, gc_deptno, gc_amount) values ( '"+ requirementNote.getAccountId()+"','"+ requirementNote.getRequirement() +"','"+ fromdate +"','N','"+ requirementNote.getDepartment() +"','"+ requirementNote.getAmount() +"' );";
    preparedStatementInsert = dbConnection.prepareStatement(insertGchargeSQL);
    preparedStatementInsert.executeUpdate();
...
<%
    ...
    if (errorList.size() > 0) {
        response.sendRedirect("newrequirementnote.jsp?acc=" + requirementNote.getAccountId()+"&bkacc="+bkacc);
    } else {
